...
const IndustryExpertDashboard = () => {
  ...
  const refreshSeminars = async () => {
    try {
      ...
      console.error('Error refreshing seminars:', err);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(`${API_URL}/api/industry-experts/profile`, formData, {
  ...
  const handleAddSeminar = async e => {
    ...
    setLoading(true);
    try {
      const res = await axios.post(`${API_URL}/api/industry-experts/seminars`, newSeminar, {
        headers: { Authorization: `Bearer ${user?.token}` },
      });
      // Update local state immediately without page reload
      // The backend now returns { success: true, data: createdSeminar }
      setSeminars([...seminars, res.data.data]);
      ...
      const errorMessage = err.response?.data?.message || 'Failed to add seminar';
      toast.error(errorMessage);
    }
    setLoading(false);
  };
  ...
  const handleDeleteSeminar = async (seminarId) => {
    ...
    setLoading(true);
    try {
      const response = await axios.delete(`${API_URL}/api/industry-experts/seminars/${seminarId}`, {
        headers: { Authorization: `Bearer ${user?.token}` },
      });
      
      if (response.data.success) {
        // Update local state immediately without page reload
        const updatedSeminars = seminars.filter(seminar => seminar._id !== seminarId);
        ...
        toast.error(response.data.message || 'Failed to delete seminar');
      }
    } catch (err) {
      console.error('Seminar deletion error:', err);
      const errorMessage = err.response?.data?.message || 'Failed to delete seminar';
      toast.error(errorMessage);
      
      // Refresh seminars from server in case of error
      refreshSeminars();
    }
    setLoading(false);
  };
          ...
          <div className="p-6">
            ...
            <nav className="space-y-2">
              ...
              <button
                ...
                className={`w-full flex items-center space-x-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors ${activeTab === 'seminars'
                    ? 'bg-white bg-opacity-20 text-white'
                    : 'text-purple-100 hover:bg-white hover:bg-opacity-10'
                  }`}
              >
              ...
              <button
                ...
                className={`w-full flex items-center space-x-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors ${activeTab === 'messages'
                    ? 'bg-white bg-opacity-20 text-white'
                    : 'text-purple-100 hover:bg-white hover:bg-opacity-10'
                  }`}
              >
                  ...
                  {seminars.length > 0 ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                      {seminars.map((seminar, index) => (
                        <div key={index} className="bg-gradient-to-br from-white to-gray-50 dark:from-gray-700 dark:to-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-600 p-4 sm:p-6 hover:shadow-md transition-shadow duration-200">
                          <div className="flex items-start justify-between mb-3 sm:mb-4">
                            <div className="flex-1 pr-2">
                              <h4 className="font-semibold text-gray-900 dark:text-white text-base sm:text-lg mb-2">
                                <TextTruncate 
                                  text={seminar.title}
                                  maxLength={30}
                                  className="block"
                            ...
                            <button 
                              ...
                              className="ml-1 p-1.5 sm:p-2 text-red-500 hover:text-red-700 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors duration-200 flex-shrink-0"
                          ...
                          <div className="space-y-2 sm:space-y-3">
                            <div className="flex items-center gap-2 text-xs sm:text-sm text-gray-600 dark:text-gray-300">
                              ...
                              <span className="truncate">{new Date(seminar.date).toLocaleDateString('en-US', { 
                                weekday: 'short', 
                                year: 'numeric', 
                                month: 'short', 
                                day: 'numeric' 
                            ...
                            <div className="flex items-center gap-2 text-xs sm:text-sm text-gray-600 dark:text-gray-300">
                              ...
                              <span className="truncate">{seminar.time}</span>
                            </div>
                            
                            <div className="flex items-center justify-between pt-2 sm:pt-3 border-t border-gray-200 dark:border-gray-600">
                              <span className="text-xs sm:text-sm text-gray-500 dark:text-gray-400">Fee:</span>
                              <span className={`font-semibold text-sm sm:text-lg ${seminar.fee === 0 ? 'text-green-600 dark:text-green-400' : 'text-gray-900 dark:text-white'}`}>
                                {seminar.fee === 0 ? 'Free' : `₹${seminar.fee}`}
                              </span>
                            </div>
                          </div>
                        </div>
                    ...
                    <div className="text-center py-12">
                      <FiVideo className="w-16 h-16 text-gray-300 dark:text-gray-600 mx-auto mb-4" />
                      <p className="text-gray-600 dark:text-gray-400 text-lg mb-2">No seminars scheduled yet</p>
                      <p className="text-gray-500 dark:text-gray-500 text-sm">Create your first seminar to get started!</p>
                    </div>
                  )}
                </div>
              </div>
