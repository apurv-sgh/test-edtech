...
const handleChangePassword = async (e) => {
  ...
  try {
    const res = await fetch('https://zegnite-backend2.onrender.com/api/users/password', {
      method: 'PUT',
      headers: {
    ...
    if (res.ok) {
      ...
      alert(data.message); // "Current password incorrect" or "User not found"
    }
  } catch (err) {
...
const handleSaveNotifications = async () => {
  try {
    const res = await fetch('https://zegnite-backend2.onrender.com/api/users/notifications', {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${localStorage.getItem('token')}`
      },
      body: JSON.stringify({ notifications }) // 'notifications' state from your component
    });
    const data = await res.json();
    if (res.ok) {
      console.log(data.message); // "Notification settings updated successfully"
...
const SettingsPage = () => {
  ...
  // Protected View for students only
//   if (user?.role !== 'student') {
//     return (
//       <div className="text-center py-20 min-h-screen">
//         <h1 className="text-3xl font-bold">Access Denied</h1>
//         <p className="mt-2 text-slate-500">Only students can access this page.</p>
            ...
            <div>
              ...
              <div className="flex items-center gap-6 p-4 bg-primary-light/50 dark:bg-slate-800/50 rounded-lg">
                <div className="relative w-24 h-24">
                  <img
y                      src={profilePicPreview || (user.profilePicture ? (user.profilePicture.startsWith('http') ? user.profilePicture : `${backendUrl}${user.profilePicture}`) : null) || user.avatar || `https://placehold.co/100x100/A78BFA/FFFFFF?text=${user.name ? user.name.charAt(0) : 'S'}`}
                    alt="Avatar"
          ...
          {activeTab === 'security' && (
            <div>
              ...
              <form className="max-w-md space-y-4">
                ...
                <button onClick={handleChangePassword} className="bg-primary text-white font-semibold py-2 px-5 rounded-lg hover:bg-primary-focus">Change Password</button>
              </form>
            </div>
          )}
          {activeTab === 'notifications' && (
            <div>
              ...
              <div className="max-w-lg divide-y divide-slate-200 dark:divide-slate-700">
                <ToggleSwitch onToggle={handleSaveNotifications} label="New Student Enrollments" description="Get notified when a new student enrolls in one of your courses." enabled={notifications.enrollments} setEnabled={() => setNotifications({...notifications, enrollments: !notifications.enrollments})}/>
                <ToggleSwitch onToggle={handleSaveNotifications} label="Assignment Submissions" description="Get notified when a student submits an assignment." enabled={notifications.submissions} setEnabled={() => setNotifications({...notifications, submissions: !notifications.submissions})}/>
                <ToggleSwitch onToggle={handleSaveNotifications} label="New Messages" description="Receive a notification for new messages in your channels." enabled={notifications.messages} setEnabled={() => setNotifications({...notifications, messages: !notifications.messages})}/>
                <ToggleSwitch onToggle={handleSaveNotifications} label="Platform Updates" description="Receive news and updates about the EdTech platform." enabled={notifications.updates} setEnabled={() => setNotifications({...notifications, updates: !notifications.updates})}/>
              </div>
            </div>
          )}
        </div>
